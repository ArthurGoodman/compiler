...
MovTest::MovTest()
    ...
    test("mov ebx,DWORD PTR [ebx+0x1]", "8b 5b 01", [this]() { c.mov(vm::EBX, vm::ptr(vm::EBX)); dump(); });
    test("mov ebx,DWORD PTR [ebx+0x100]", "8b 9b 00 01 00 00", [this]() { c.mov(vm::EBX, vm::EBX + (byte)0x1); dump(); });
    ...
    test("mov ebx,0x1", "bb 01 00 00 00", [this]() { c.mov(vm::EBX, (byte)0x1); dump(); });
    ...
    test("mov DWORD PTR [ebx],0x1", "c7 03 01 00 00 00", [this]() { c.mov(vm::ptr(vm::EBX), (byte)0x1); dump(); });
    test("mov DWORD PTR [ebx+0x1],0x1", "c7 43 01 01 00 00 00", [this]() { c.mov(vm::EBX + (byte)0x1, (byte)0x1); dump(); });
    test("mov DWORD PTR [ebx+0x100],0x1", "c7 83 00 01 00 00 01 00 00 00", [this]() { c.mov(vm::EBX + 0x100, (byte)0x1); dump(); });
